  ...
  if (other->collision_layer == COLLISION_LAYER_ENEMY) {
    for (usize i = 0; i < entity_count(); ++i) {
      ...
      if (entity->body_id == hit.other_id) {
        ...
        spawn_enemy(true, true, is_flipped);
...
int main(int argc, char * argv[]) {
  ...
  i32 window_render_width, window_render_height;
  SDL_GetWindowSize(window, &window_render_width, &window_render_height);
  render_width = window_render_width / render_get_scale();
  render_height = window_render_height / render_get_scale();
